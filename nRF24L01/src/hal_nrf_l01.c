...
uint8_t hal_nrf_get_clear_irq_flags()
{
...
void hal_nrf_set_crc_mode( hal_nrf_crc_mode_t crc_mode )
{
    hal_nrf_write_reg( CONFIG,
                       ( hal_nrf_read_reg ( CONFIG ) & ~ ( BIT_3 | BIT_2 ) ) | ( UINT8 ( crc_mode ) << 2 ) );
}
...
void hal_nrf_open_pipe( hal_nrf_address_t pipe_num, bool auto_ack )
{
    switch ( pipe_num )
    {
        ...
        default:
            // Debugging message might be good here
            break;
    }
}
...
uint8_t hal_nrf_get_auto_retr_status()
{
    return hal_nrf_read_reg ( OBSERVE_TX );
}

uint8_t hal_nrf_get_packet_lost_ctr()
{
    return ( hal_nrf_read_reg( OBSERVE_TX ) & ( BIT_7 | BIT_6 | BIT_5 | BIT_4 ) ) >> 4;
}

uint8_t hal_nrf_get_address_width()
{
...
uint8_t hal_nrf_get_rx_data_source()
{
